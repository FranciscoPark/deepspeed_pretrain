...
def tokenize_function(examples, tokenizer, text_column,block_size):
    if text_column not in examples:
        print(f"Missing text column: {text_column}")
        print(f"Available keys: {examples.keys()}")
    output = tokenizer(examples[text_column], truncation=True, padding="max_length", max_length=block_size)
    return output
